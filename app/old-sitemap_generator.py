...
def get_articles(self, from_date: str, to_date: str, page: int = 1, page_size: int = 50) -> List[Dict]:
    ...
    try:
        ...
        # Debug: print first article to see structure
        if data and isinstance(data, (list, dict)):
            first_article = data[0] if isinstance(data, list) else (data.get('results') or data.get('data'))[0]
            print(f"First article structure: {list(first_article.keys())}")
            print(f"First article datetime fields: last_published_at={first_article.get('last_published_at')}, updated_at={first_article.get('updated_at')}, created_at={first_article.get('created_at')}")
    ...
    def build_url_path(self, article: Dict) -> str:
        ...
        # Based on your sitemap examples, the structure varies
        
        # Try to get from article data first
